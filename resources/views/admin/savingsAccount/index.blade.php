    ...
    <table class="table">
        ...
        @foreach ($viewData['savingsAccounts'] as $savingsAccount)
            <tr>
                ...
                <td>
                    <a href="{{ route('admin.savingsAccount.show', ['id' => $savingsAccount->getId()]) }}" class="btn btn-info">
                        {{ __('messages.viewButton') }}
                    </a>
                    <form action="{{ route('admin.savingsAccount.destroy', ['id' => $savingsAccount->getId()]) }}" method="POST" style="display:inline;">
                        @csrf
                        @method('DELETE')
                    ...
                    <a href="{{ route('admin.savingsAccount.edit', ['id' => $savingsAccount->getId()]) }}" class="btn btn-warning">
